...
def download_stream(url: str, chunk_size: int = 4 * 1024 * 1024) -> Iterator[bytes]:
    ...
    Note: Fixed-size chunking is now handled by the vault layer.
    """
    ...
    # Use format selection that works well with stdout
    # Merging to stdout is tricky, so we prefer a single file if possible
    cmd = [
        ytdlp_path,
        "-o", "-", # Output to stdout
        "-f", "best[ext=mp4]/best",
